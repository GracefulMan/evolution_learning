...
class Evaluator:
    ...
    def evaluate_save(self, agent, steps, log_tuple) -> bool:
        self.total_step += steps  # update total training steps

        if time.time() - self.eval_time > self.eval_gap:
            self.eval_time = time.time()

            rewards_steps_list = [get_episode_return(self.env, agent.act, self.device) for _ in range(self.eval_times1)]
            r_avg, r_std, s_avg, s_std = self.get_r_avg_std_s_avg_std(rewards_steps_list)

            if r_avg > self.r_max:  # evaluate actor twice to save CPU Usage and keep precision
                rewards_steps_list += [get_episode_return(self.env, agent.act, self.device)
            ...
            if r_avg > self.r_max:  # save checkpoint with highest episode return
                ...
                critic_save_path = f'{self.cwd}/critic.pth'
                torch.save(agent.act.state_dict(), act_save_path)
                torch.save(agent.cri.state_dict(), critic_save_path)
...
def train_and_evaluate(args):
    ...
    if os.path.exists(f'{cwd}/actor.pth') and os.path.exists(f'{cwd}/critic.pth'):
        agent.act.load_state_dict(torch.load(f'{cwd}/actor.pth'))
        agent.cri.load_state_dict(torch.load(f'{cwd}/critic.pth'))
        print('load agent actor and critic params.')
    ...
    while if_train:
        ...
        with torch.no_grad():
            if_reach_goal = evaluator.evaluate_save(agent, steps, logging_tuple)
